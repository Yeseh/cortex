...
const createMemoryFile = async (
    ...
    const result = await adapter.writeMemoryFile(
        slugPath, serialized.value, {
...
const _createCategoryIndex = async (
    ...
    const result = await adapter.writeIndexFile(
        categoryPath, serialized.value,
    );
    if (!result.ok) {
...
const getTextContent = (content: { uri: string; text?: string; blob?: string } | undefined): string => {
    if (!content) {
...
describe(
    'buildResourceUri', () => {
        it(
            'should build URI for memory content (no trailing slash)', () => {
                const uri = buildResourceUri(
                    'default', 'project/my-memory', false,
                );
        ...
        it(
            'should build URI for category listing (with trailing slash)', () => {
                const uri = buildResourceUri(
        ...
        it(
            'should handle root category listing', () => {
                const uri = buildResourceUri(
                    'default', '', true,
                );
        ...
        it(
            'should handle nested category paths', () => {
                const uri = buildResourceUri(
                    'default', 'project/subcategory', true,
                );
...
describe(
    'parseUriVariables', () => {
        ...
        it(
            'should parse store and path from variables', () => {
                const variables = { store: 'my-store', 'path*': 'project/my-memory' };
                const result = parseUriVariables(
                    variables, config,
                );
        ...
        it(
            'should use default store when store is not provided', () => {
        ...
        it(
            'should detect category listing when path ends with slash', () => {
        ...
        it(
            'should detect category listing when path is empty', () => {
...
describe(
    'resolveStoreRoot', () => {
        it(
            'should resolve store root with provided store name', () => {
        ...
        it(
            'should use default store when store name is undefined', () => {
                const config = createTestConfig('/data/cortex');
                const result = resolveStoreRoot(
                    config, undefined,
                );
...
describe(
    'readMemoryContent', () => {
        ...
        it(
            'should retrieve memory content at a valid path', async () => {
                await createMemoryFile(
                    storeRoot, 'project/test-memory', {
                        frontmatter: {
                ...
                const result = await readMemoryContent(
                    adapter, 'default', 'project/test-memory',
                );
        ...
        it(
            'should return correct MIME type (text/plain)', async () => {
                await createMemoryFile(
                    storeRoot, 'project/mime-test', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: [],
                ...
                const result = await readMemoryContent(
                    adapter, 'default', 'project/mime-test',
                );
        ...
        it(
            'should return correct URI in response', async () => {
                await createMemoryFile(
                    storeRoot, 'project/uri-test', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: [],
                ...
                const result = await readMemoryContent(
                    adapter, 'my-store', 'project/uri-test',
                );
        ...
        it(
            'should handle nested category paths', async () => {
                await createMemoryFile(
                    storeRoot, 'project/subcategory/deep-memory', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: ['nested'],
        ...
        it(
            'should return error for non-existent memory', async () => {
                const result = await readMemoryContent(
                    adapter, 'default', 'project/non-existent',
                );
        ...
        it(
            'should return error for invalid memory path format', async () => {
                const result = await readMemoryContent(
                    adapter, 'default', 'invalid',
                );
...
describe(
    'readCategoryListing', () => {
        ...
        it(
            'should list memories in a category', async () => {
                // Create memories which automatically create the index
                await createMemoryFile(
                    storeRoot, 'project/memory-1', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: [],
                ...
                const result = await readCategoryListing(
                    adapter, 'default', 'project',
                );
        ...
        it(
            'should list subcategories', async () => {
                // Create a memory in a subcategory
                await createMemoryFile(
                    storeRoot, 'project/subcategory/sub-memory', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: [],
        ...
        it(
            'should return correct JSON structure', async () => {
                await createMemoryFile(
                    storeRoot, 'project/structure-test', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: ['tag1'],
        ...
        it(
            'should return correct MIME type (application/json)', async () => {
                await createMemoryFile(
                    storeRoot, 'project/mime-test', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
        ...
        it(
            'should handle root category listing (empty path)', async () => {
                // Create memories in different root categories
                await createMemoryFile(
                    storeRoot, 'project/root-memory', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: [],
        ...
        it(
            'should return error for non-existent category', async () => {
                const result = await readCategoryListing(
                    adapter, 'default', 'non-existent-category',
                );
        ...
        it(
            'should include URI for each memory in listing', async () => {
                await createMemoryFile(
                    storeRoot, 'project/uri-memory', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: [],
        ...
        it(
            'should include URI for each subcategory in listing', async () => {
                await createMemoryFile(
                    storeRoot, 'project/sub/memory', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: [],
...
describe(
    'listResources', () => {
        ...
        it(
            'should list available resources', async () => {
        ...
        it(
            'should include root store resource', async () => {
                const storeRoot = join(
                    testDir, 'default',
                );
                await mkdir(
        ...
        it(
            'should include category resources', async () => {
                const storeRoot = join(
                    testDir, 'default',
                );
                await mkdir(
                ...
                await createMemoryFile(
                    storeRoot, 'project/category-test', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: [],
                ...
                expect(result.ok).toBe(true);
                if (result.ok) {
                    const categoryResource = result.value.resources.find((r) => r.name === 'Category: project');
        ...
        it(
            'should include memory resources', async () => {
                const storeRoot = join(
                    testDir, 'default',
                );
                await mkdir(
                ...
                await createMemoryFile(
                    storeRoot, 'project/memory-resource', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: [],
                ...
                expect(result.ok).toBe(true);
                if (result.ok) {
                    const memoryResource = result.value.resources.find((r) => r.name === 'Memory: project/memory-resource');
        ...
        it(
            'should include subcategory resources', async () => {
                const storeRoot = join(
                    testDir, 'default',
                );
                await mkdir(
                ...
                await createMemoryFile(
                    storeRoot, 'project/sub/nested-memory', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: [],
                ...
                expect(result.ok).toBe(true);
                if (result.ok) {
                    const subResource = result.value.resources.find((r) => r.name === 'Category: project/sub');
        ...
        it(
            'should return empty resources when no memories exist', async () => {
                const storeRoot = join(
                    testDir, 'default',
                );
                await mkdir(
        ...
        it(
            'should list resources from multiple root categories', async () => {
                const storeRoot = join(
                    testDir, 'default',
                );
                await mkdir(
                ...
                await createMemoryFile(
                    storeRoot, 'project/project-memory', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: [],
                ...
                await createMemoryFile(
                    storeRoot, 'persona/persona-memory', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: [],
...
describe(
    'constants', () => {
        ...
        it(
            'should export MEMORY_URI_SCHEME', () => {
...
describe(
    'error handling', () => {
        ...
        it(
            'should handle memory path with special characters in slug', async () => {
                // The path validation should reject invalid characters
                const result = await readMemoryContent(
                    adapter, 'default', 'project/invalid..path',
                );
        ...
        it(
            'should handle path traversal attempts', async () => {
                const result = await readMemoryContent(
                    adapter, 'default', 'project/../../../etc/passwd',
                );
        ...
        it(
            'should handle empty store name', async () => {
                const config = createTestConfig(testDir);
                const result = parseUriVariables(
                    { store: '', 'path*': 'project/memory' }, config,
                );
...
describe(
    'resource workflow', () => {
        ...
        it(
            'should support full browse workflow: list -> category -> memory', async () => {
                // Create test data
                await createMemoryFile(
                    storeRoot, 'project/workflow-memory', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: ['workflow'],
                ...
                const categoryResult = await readCategoryListing(
                    adapter, 'default', 'project',
                );
                ...
                const memoryResult = await readMemoryContent(
                    adapter, 'default', 'project/workflow-memory',
                );
        ...
        it(
            'should handle deeply nested category structure', async () => {
                await createMemoryFile(
                    storeRoot, 'project/level1/level2/deep-memory', {
                        frontmatter: {
                            createdAt: new Date('2024-01-01'),
                            updatedAt: new Date('2024-01-02'),
                            tags: ['deep'],
                ...
                // Check each level
                const level1Result = await readCategoryListing(
                    adapter, 'default', 'project',
                );
                expect(level1Result.ok).toBe(true);

                const level2Result = await readCategoryListing(
                    adapter, 'default', 'project/level1',
                );
