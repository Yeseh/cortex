...
describe('health endpoint', () => {
    ...
    describe('config handling', () => {
        it('should use provided config dataPath', async () => {
            ...
            const app = express();
            app.use('/health', createHealthRouter(customConfig));
            const { server: s, baseUrl } = await startServer(app);
            server = s;
        ...
        it('should use different dataPath for different configs', async () => {
            ...
            const app1 = express();
            app1.use('/health', createHealthRouter(config1));
            const { server: s1, baseUrl: baseUrl1 } = await startServer(app1);

            const app2 = express();
            app2.use('/health', createHealthRouter(config2));
...
`;
        ...
        it('should return healthy when dataPath directory does not exist', async () => {
            ...
            const app = express();
            app.use('/health', createHealthRouter(configWithBadPath));
            const { server: s, baseUrl } = await startServer(app);
            server = s;
