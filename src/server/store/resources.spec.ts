...
describe('Store Resources', () => {
    ...
    describe('registerStoreResources function', () => {
        it('should register resources without throwing', () => {
            expect(() => registerStoreResources(server, config)).not.toThrow();
        });
        ...
        it('should call server.registerResource with store-list', () => {
            ...
            spyOn(server, 'registerResource').mockImplementation(
    ...
    describe('Store List Resource (cortex://store/)', () => {
        ...
        it('should throw McpError when data path is inaccessible', async () => {
            ...
            registerStoreResources(server, invalidConfig);

            const registeredResources = getRegisteredResources(server);
            const storeListResource = registeredResources['cortex://store/'];

            await expect(async () => {
                await storeListResource!.readCallback(new URL('cortex://store/'), {});
            }).toThrow(McpError);
        });
    ...
    describe('Store Detail Resource (cortex://store/{name})', () => {
        ...
        it('should return categories list for store with subdirectories', async () => {
            // Create store with nested subdirectories
            await fs.mkdir(path.join(testDir, 'project-store'));
            await fs.mkdir(path.join(testDir, 'project-store', 'docs'));
            await fs.mkdir(path.join(testDir, 'project-store', 'code'));
            await fs.mkdir(path.join(testDir, 'project-store', 'tests'));
        ...
        it('should return empty categories for store with no subdirectories', async () => {
            // Create empty store
            await fs.mkdir(path.join(testDir, 'empty-store'));
        ...
        it('should return empty categories when store contains only files', async () => {
            // Create store with only files
            await fs.mkdir(path.join(testDir, 'files-only-store'));
            await fs.writeFile(path.join(testDir, 'files-only-store', 'file1.txt'), 'content');
            await fs.writeFile(path.join(testDir, 'files-only-store', 'file2.md'), 'content');
    ...
    describe('Resource Template List Callback', () => {
        it('should return all stores as discoverable resources', async () => {
            // Create some stores
            await fs.mkdir(path.join(testDir, 'store-alpha'));
            await fs.mkdir(path.join(testDir, 'store-beta'));
            await fs.mkdir(path.join(testDir, 'store-gamma'));
        ...
        it('should throw McpError when listStores fails', async () => {
        ...
        it('should only include directories in resource list', async () => {
            // Create mix of files and directories
            await fs.mkdir(path.join(testDir, 'real-store-1'));
            await fs.mkdir(path.join(testDir, 'real-store-2'));
            await fs.writeFile(path.join(testDir, 'not-a-store.txt'), 'content');
    ...
    describe('Resource Template Complete Callback', () => {
        ...
        it('should return empty array for autocomplete when listStores fails', async () => {
            const filePath = path.join(testDir, 'not-a-directory');
            await fs.writeFile(filePath, 'content');
    ...
    describe('getStoreCategories (tested via store-detail resource)', () => {
        it('should successfully list subdirectories', async () => {
            await fs.mkdir(path.join(testDir, 'test-store'));
            await fs.mkdir(path.join(testDir, 'test-store', 'subdir-1'));
            await fs.mkdir(path.join(testDir, 'test-store', 'subdir-2'));
            await fs.mkdir(path.join(testDir, 'test-store', 'subdir-3'));
        ...
        it('should filter out files and only return directories', async () => {
            await fs.mkdir(path.join(testDir, 'mixed-store'));
            await fs.mkdir(path.join(testDir, 'mixed-store', 'actual-category'));
            await fs.writeFile(path.join(testDir, 'mixed-store', 'some-file.txt'), 'content');
            await fs.writeFile(path.join(testDir, 'mixed-store', 'another-file.md'), 'content');
    ...
    describe('Result type structure', () => {
        ...
        it('should throw McpError for store list failure', async () => {
            const filePath = path.join(testDir, 'not-a-directory');
            await fs.writeFile(filePath, 'content');
