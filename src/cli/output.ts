...
type SerializeResult = Result<string, OutputSerializeError>;
type JsonRecordResult = Result<Record<string, unknown>, OutputSerializeError>;
type JsonRecordArrayResult = Result<Record<string, unknown>[], OutputSerializeError>;
...
export const toonOptions = {
    delimiter: '\t', // Tab delimiters for maximum token efficiency
    keyFolding: 'safe', // Collapse nested paths into dotted notation
} as const;
...
const validateStoreName = (value: string, field: string): Result<string, OutputSerializeError> => {
    const trimmedValue = typeof value === 'string' ? value.trim() : '';
    if (!trimmedValue) {
        return err({
            code: 'INVALID_FIELD',
...
const serializeTimestamp = (value: Date, field: string): Result<string, OutputSerializeError> => {
...
const serializeTags = (tags: string[]): Result<string[], OutputSerializeError> => {
...
const serializeCategoryMemoriesJson = (entries: OutputCategoryMemory[]): JsonRecordArrayResult => {
...
const buildSubcategoryJsonEntry = (entry: OutputSubcategory): JsonRecordResult => {
    const entryPath = validateRequiredPath(
        entry.path,
        'subcategories.path',
        'Category subcategory'
    ...
    const memoryCount = validateNonNegativeNumber(entry.memoryCount, 'subcategories.memory_count');
    if (!memoryCount.ok) {
        return memoryCount;
    }
...
const buildMemoryYamlLines = (
    ...
    const tokenEstimate = writeOptionalYamlNumber(lines, 'token_estimate', metadata.tokenEstimate);
    if (!tokenEstimate.ok) {
        return tokenEstimate;
    }
...
const serializeMemoryYaml = (memory: OutputMemory): Result<string, OutputSerializeError> => {
    ...
    const lines = buildMemoryYamlLines(memoryPath.value, created.value, memory.metadata);
    if (!lines.ok) {
        return lines;
    }
...
const serializeCategoryYaml = (category: OutputCategory): Result<string, OutputSerializeError> => {
    ...
    const subcategoriesResult = appendCategorySubcategories(lines, category.subcategories);
...
const buildMemoryJsonMetadata = (memory: OutputMemory): JsonRecordResult => {
    ...
    const tokenEstimate = serializeOptionalNumber(memory.metadata.tokenEstimate, 'token_estimate');
    if (!tokenEstimate.ok) {
        return tokenEstimate;
    }
    if (tokenEstimate.value !== undefined) {
...
const buildMemoryJsonBase = (memory: OutputMemory): JsonRecordResult => {
    const created = serializeTimestamp(memory.metadata.createdAt, 'created_at');
    if (!created.ok) {
        return created;
    }
...
const serializeMemoryJson = (memory: OutputMemory): Result<string, OutputSerializeError> => {
    const memoryPath = validateRequiredPath(memory.path, 'path', 'Memory');
    if (!memoryPath.ok) {
        return memoryPath;
    }
...
const serializeMemoryToon = (memory: OutputMemory): Result<string, OutputSerializeError> => {
    const memoryPath = validateRequiredPath(memory.path, 'path', 'Memory');
    if (!memoryPath.ok) {
        return memoryPath;
    }
...
const serializeCategoryJson = (category: OutputCategory): Result<string, OutputSerializeError> => {
    const categoryPath = validateRequiredPath(category.path, 'path', 'Category');
    if (!categoryPath.ok) {
        return categoryPath;
    }
    ...
    const subcategoriesResult = serializeCategorySubcategoriesJson(category.subcategories);
    if (!subcategoriesResult.ok) {
        return subcategoriesResult;
    }
...
const serializeCategoryToon = (category: OutputCategory): Result<string, OutputSerializeError> => {
    const categoryPath = validateRequiredPath(category.path, 'path', 'Category');
    if (!categoryPath.ok) {
        return categoryPath;
    }
...
const serializeStoreYaml = (store: OutputStore): Result<string, OutputSerializeError> => {
...
const serializeStoreJson = (store: OutputStore): Result<string, OutputSerializeError> => {
    const name = validateStoreName(store.name, 'store.name');
    if (!name.ok) {
        return name;
    }
...
const serializeStoreToon = (store: OutputStore): Result<string, OutputSerializeError> => {
    const name = validateStoreName(store.name, 'store.name');
    if (!name.ok) {
        return name;
    }
...
const serializeStoreRegistryYaml = (registry: OutputStoreRegistry): SerializeResult => {
...
const serializeStoreRegistryJson = (registry: OutputStoreRegistry): SerializeResult => {
    if (!Array.isArray(registry.stores)) {
        return err({
            code: 'INVALID_FIELD',
            message: 'Stores must be an array.',
    ...
    const stores: { name: string; path: string }[] = [];
    for (const entry of registry.stores) {
        const name = validateStoreName(entry.name, 'stores.name');
        if (!name.ok) {
            return name;
...
const serializeStoreRegistryToon = (registry: OutputStoreRegistry): SerializeResult => {
    if (!Array.isArray(registry.stores)) {
        return err({
            code: 'INVALID_FIELD',
            message: 'Stores must be an array.',
...
const serializeStoreInitYaml = (storeInit: OutputStoreInit): SerializeResult => {
...
const serializeStoreInitJson = (storeInit: OutputStoreInit): SerializeResult => {
    const path = validateRequiredPath(storeInit.path, 'path', 'Store');
    if (!path.ok) {
        return path;
    }
...
const serializeStoreInitToon = (storeInit: OutputStoreInit): SerializeResult => {
    const path = validateRequiredPath(storeInit.path, 'path', 'Store');
    if (!path.ok) {
        return path;
    }
...
const serializeToonOutput = (payload: OutputPayload): Result<string, OutputSerializeError> => {
    switch (payload.kind) {
        case 'memory':
            return serializeMemoryToon(payload.value);
        case 'category':
            return serializeCategoryToon(payload.value);
        case 'store':
            return serializeStoreToon(payload.value);
        case 'store-registry':
...
export const serializeOutput = (
    ...
    if (format === 'toon') {
        return serializeToonOutput(payload);
    }
