...
const TEST_ROOT = join(
    import.meta.dir, '../../../.test-prune-command',
);
...
describe(
    'runPruneCommand', () => {
        ...
        test(
            'returns no pruned when no expired memories exist', async () => {
                await createMemoryFile('project/active-memory');
                await createIndex(
                    'project', [{ path: 'project/active-memory', tokenEstimate: 10 }],
                );
        ...
        test(
            'deletes expired memories', async () => {
                ...
                const memoryPath = join(
                    TEST_ROOT, 'memories', 'project', 'expired-memory.md',
                );
        ...
        test(
            'dry run does not delete files', async () => {
                const pastDate = new Date(Date.now() - 86400000);
        ...
        test(
            'keeps non-expired memories', async () => {
                const pastDate = new Date(Date.now() - 86400000);
                ...
                const activePath = join(
                    TEST_ROOT, 'memories', 'project', 'active-memory.md',
                );
        ...
        test(
            'rejects positional arguments', async () => {
                const result = await runPruneCommand({
                    storeRoot: TEST_ROOT,
                    args: ['some-arg'],
                });
