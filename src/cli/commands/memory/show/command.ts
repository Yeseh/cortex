...
export async function handleShow(
    ...
    if (!options.includeExpired && parsed.value.frontmatter.expiresAt) {
        const now = new Date();
        if (parsed.value.frontmatter.expiresAt.getTime() <= now.getTime()) {
    ...
    const outputMemory: OutputMemory = {
        ...
        metadata: {
            ...
            updatedAt: parsed.value.frontmatter.updatedAt,
