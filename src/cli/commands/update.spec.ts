...
describe('update CLI command', () => {
    ...
    const createMemoryFile = async (slugPath: string, content: string) => {
        const memoryDir = join(tempDir, 'memories', ...slugPath.split('/').slice(0, -1));
        await mkdir(memoryDir, { recursive: true });
        const filePath = join(tempDir, 'memories', `${slugPath}.md`);
    ...
    beforeEach(async () => {
        tempDir = await mkdtemp(join(tmpdir(), 'cortex-update-cli-'));
    });
    ...
    describe('success cases', () => {
        it('should update memory content with --content flag', async () => {
            const slugPath = 'project/test-memory';
            await createMemoryFile(slugPath, buildMemoryContent({}));

            const now = new Date('2024-06-15T12:00:00.000Z');
            const result = await runUpdateCommand(
                buildOptions([slugPath, '--content', 'Updated content!'], { now })
            );
        ...
        it('should update memory content with --file flag', async () => {
            ...
            const result = await runUpdateCommand(
                buildOptions([slugPath, '--file', inputPath], { now })
            );
        ...
        it('should update memory tags with --tags flag', async () => {
            ...
            const now = new Date('2024-06-15T12:00:00.000Z');
            const result = await runUpdateCommand(
                buildOptions([slugPath, '--tags', 'new-tag,another-tag'], { now })
            );
        ...
        it('should update memory expiry with --expires-at flag', async () => {
            const slugPath = 'project/expiring-memory';
            await createMemoryFile(slugPath, buildMemoryContent({}));

            const now = new Date('2024-06-15T12:00:00.000Z');
            const result = await runUpdateCommand(
                buildOptions([slugPath, '--expires-at', '2025-12-31T23:59:59.000Z'], { now })
            );
        ...
        it('should clear memory expiry with --clear-expiry flag', async () => {
            ...
            const now = new Date('2024-06-15T12:00:00.000Z');
            const result = await runUpdateCommand(
                buildOptions([slugPath, '--clear-expiry'], { now })
            );
        ...
        it('should preserve fields not being updated', async () => {
            ...
            const now = new Date('2024-06-15T12:00:00.000Z');
            const result = await runUpdateCommand(
                buildOptions([slugPath, '--tags', 'updated-tag'], { now })
            );
        ...
        it('should handle deeply nested category paths', async () => {
            ...
            const now = new Date('2024-06-15T12:00:00.000Z');
            const result = await runUpdateCommand(
                buildOptions([slugPath, '--content', 'Updated deep content'], { now })
            );
        ...
        it('should replace tags with empty array when empty string provided', async () => {
            const slugPath = 'project/clear-tags';
            await createMemoryFile(slugPath, buildMemoryContent({ tags: ['tag1', 'tag2'] }));

            const now = new Date('2024-06-15T12:00:00.000Z');
            const result = await runUpdateCommand(buildOptions([slugPath, '--tags', ''], { now }));
    ...
    describe('error cases', () => {
        it('should return INVALID_ARGS when memory path is missing', async () => {
            const result = await runUpdateCommand(buildOptions(['--content', 'Hello']));
        ...
        it('should return INVALID_ARGS when no updates are provided', async () => {
        ...
        it('should return INVALID_ARGS for unknown flag', async () => {
            const result = await runUpdateCommand(
                buildOptions(['project/memory', '--unknown', 'value'])
            );
        ...
        it('should return INVALID_ARGS when --content flag has no value', async () => {
            const result = await runUpdateCommand(buildOptions(['project/memory', '--content']));
        ...
        it('should return INVALID_ARGS when --file flag has no value', async () => {
            const result = await runUpdateCommand(buildOptions(['project/memory', '--file']));
        ...
        it('should return INVALID_ARGS when --tags flag has no value', async () => {
            const result = await runUpdateCommand(buildOptions(['project/memory', '--tags']));
        ...
        it('should return INVALID_ARGS when --expires-at flag has no value', async () => {
            const result = await runUpdateCommand(buildOptions(['project/memory', '--expires-at']));
        ...
        it('should return INVALID_ARGS for invalid expires-at timestamp', async () => {
            const result = await runUpdateCommand(
                buildOptions(['project/memory', '--expires-at', 'not-a-date'])
            );
        ...
        it('should return INVALID_ARGS when both --expires-at and --clear-expiry are provided', async () => {
            const result = await runUpdateCommand(
                buildOptions([
                    'project/memory',
                    '--expires-at',
        ...
        it('should return INVALID_ARGS for tags with empty parts', async () => {
            const slugPath = 'project/memory';
            await createMemoryFile(slugPath, buildMemoryContent({}));

            const result = await runUpdateCommand(buildOptions([slugPath, '--tags', 'foo,,bar']));
        ...
        it('should return INVALID_ARGS for too many positional arguments', async () => {
            const result = await runUpdateCommand(
                buildOptions(['project/memory', 'extra-arg', '--content', 'Hello'])
            );
        ...
        it('should return INVALID_PATH for invalid slug path (missing category)', async () => {
            const result = await runUpdateCommand(
                buildOptions(['single-segment', '--content', 'Hello'])
            );
        ...
        it('should return INVALID_PATH for invalid slug characters', async () => {
            const result = await runUpdateCommand(
                buildOptions(['Project/Invalid_Memory', '--content', 'Hello'])
            );
        ...
        it('should return MEMORY_NOT_FOUND when memory does not exist', async () => {
            const result = await runUpdateCommand(
                buildOptions(['project/nonexistent', '--content', 'Hello'])
            );
        ...
        it('should return CONTENT_INPUT_FAILED when file does not exist', async () => {
            const slugPath = 'project/memory';
            await createMemoryFile(slugPath, buildMemoryContent({}));

            const result = await runUpdateCommand(
                buildOptions([slugPath, '--file', '/nonexistent/file.txt'])
            );
        ...
        it('should return PARSE_FAILED for malformed memory file', async () => {
            const slugPath = 'project/malformed';
            await createMemoryFile(slugPath, 'This file has no frontmatter.');

            const result = await runUpdateCommand(
                buildOptions([slugPath, '--content', 'New content'])
            );
    ...
    describe('edge cases', () => {
        it('should ignore empty string arguments', async () => {
            ...
            const now = new Date('2024-06-15T12:00:00.000Z');
            const result = await runUpdateCommand(
                buildOptions(['', slugPath, '', '--content', 'Hello', ''], { now })
            );
        ...
        it('should handle content with special characters', async () => {
            ...
            const result = await runUpdateCommand(
                buildOptions([slugPath, '--content', specialContent], { now })
            );
        ...
        it('should trim tag values', async () => {
            ...
            const now = new Date('2024-06-15T12:00:00.000Z');
            const result = await runUpdateCommand(
                buildOptions([slugPath, '--tags', ' foo , bar '], { now })
            );
