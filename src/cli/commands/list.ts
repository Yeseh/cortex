...
import { parseCategoryIndex } from "../../index/parser.ts";
...
const parseListArgs = (args: string[]): Result<ParsedListArgs, ListCommandError> => {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (value === "--format") {
      ...
      if (!next || (next !== "yaml" && next !== "json")) {
        return err({
          code: "INVALID_ARGUMENTS",
          message: "--format requires 'yaml' or 'json'.",
        });
      }
      format = next;
      index += 1;
...
): Promise<Result<ListCommandOutput, ListCommandError>> => {
