    ...
    'reindex CLI command', () => {
        ...
        const readIndexFile = async (categoryPath: string): Promise<string | null> => {
            const indexPath = join(
                tempDir, 'indexes', `${categoryPath}.yml`,
            );
            try {
        ...
        describe(
            'success cases', () => {
                it(
                    'should reindex category indexes for single memory', async () => {
                        await createMemoryFile(
                            'project/test-memory', buildMemoryContent({}),
                        );
                ...
                it(
                    'should reindex category indexes for multiple memories in same category', async () => {
                        await createMemoryFile(
                            'project/memory-a', buildMemoryContent({}),
                        );
                        await createMemoryFile(
                            'project/memory-b', buildMemoryContent({}),
                        );
                        await createMemoryFile(
                            'project/memory-c', buildMemoryContent({}),
                        );
                ...
                it(
                    'should reindex category indexes for memories in multiple categories', async () => {
                        await createMemoryFile(
                            'project-a/memory-1', buildMemoryContent({}),
                        );
                        await createMemoryFile(
                            'project-b/memory-2', buildMemoryContent({}),
                        );
                ...
                it(
                    'should reindex deeply nested categories', async () => {
                        await createMemoryFile(
                            'a/b/c/deep-memory', buildMemoryContent({}),
                        );
                ...
                it(
                    'should track subcategories in parent indexes', async () => {
                        await createMemoryFile(
                            'parent/child/memory', buildMemoryContent({}),
                        );
                ...
                it(
                    'should handle empty store gracefully', async () => {
                        // No memories created
                        const result = await runReindexCommand(buildOptions([]));
                ...
                it(
                    'should include memory metadata in index', async () => {
                        await createMemoryFile(
                ...
                it(
                    'should replace existing indexes completely', async () => {
                        // First create a memory and index it
                        await createMemoryFile(
                            'project/first-memory', buildMemoryContent({}),
                        );
                        ...
                        await rm(join(
                            tempDir, 'memories', 'project', 'first-memory.md',
                        ));

                        // Add a different memory
                        await createMemoryFile(
                            'project/second-memory', buildMemoryContent({}),
                        );
        ...
        describe(
            'error cases', () => {
                it(
                    'should return INVALID_ARGUMENTS when extra arguments are provided', async () => {
                ...
                it(
                    'should return INVALID_ARGUMENTS for multiple extra arguments', async () => {
                        const result = await runReindexCommand(buildOptions([
                            'arg1',
                            'arg2',
                            'arg3',
                ...
                it(
                    'should return INVALID_ARGUMENTS for flag arguments', async () => {
        ...
        describe(
            'edge cases', () => {
                it(
                    'should handle memory with special content characters', async () => {
                        await createMemoryFile(
                            'project/special',
                            buildMemoryContent({
                ...
                it(
                    'should handle many memories efficiently', async () => {
                ...
                it(
                    'should handle slug paths with hyphens', async () => {
                        await createMemoryFile(
                            'my-project/sub-category/my-memory', buildMemoryContent({}),
                        );
