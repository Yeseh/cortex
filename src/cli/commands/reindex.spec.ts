...
describe('reindex CLI command', () => {
    ...
    const readIndexFile = async (categoryPath: string): Promise<string | null> => {
        const indexPath = join(tempDir, 'indexes', `${categoryPath}.yml`);
        try {
    ...
    describe('success cases', () => {
        ...
        it('should replace existing indexes completely', async () => {
            ...
            await rm(join(tempDir, 'memories', 'project', 'first-memory.md'));
    ...
    describe('error cases', () => {
        ...
        it('should return INVALID_ARGUMENTS for multiple extra arguments', async () => {
            const result = await runReindexCommand(buildOptions(['arg1', 'arg2', 'arg3']));

            expect(result.ok).toBe(false);
            if (result.ok) return;

            expect(result.error.code).toBe('INVALID_ARGUMENTS');
            expect(result.error.details?.args).toEqual(['arg1', 'arg2', 'arg3']);
        });
