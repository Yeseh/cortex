...
import { serializeCategoryIndex } from "../../index/parser.ts";
...
const loadRegistryOrEmpty = async (
  registryPath: string
): Promise<Result<Record<string, { path: string }>, StoreCommandError>> => {
...
const runStoreList = async (
  options: StoreCommandOptions
): Promise<Result<StoreCommandResult, StoreCommandError>> => {
...
const runStoreAction = (
  ...
  switch (command) {
    case "list":
      return runStoreList(options);
    case "add": {
      const [name, path] = args;
      return runStoreAddCommand(options, name, path);
    }
    case "remove": {
      const [name] = args;
      return runStoreRemoveCommand(options, name);
    }
    case "init": {
      const [path] = args;
      return runStoreInit(options, path);
    }
    default:
