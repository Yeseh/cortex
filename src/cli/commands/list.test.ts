...
const TEST_ROOT = join(import.meta.dir, "../../../.test-list-command");

const createMemoryFile = async (
  slugPath: string,
  options: { expiresAt?: Date } = {}
): Promise<void> => {
  const filePath = join(TEST_ROOT, "memories", `${slugPath}.md`);
...
const createIndex = async (
  ...
  const filePath = join(TEST_ROOT, "indexes", `${category}.yml`);
  const dir = filePath.split(/[/\\]/).slice(0, -1).join("/");
  await mkdir(dir, { recursive: true });

  const lines = ["memories:"];
  for (const memory of memories) {
    lines.push("  -", `    path: ${memory.path}`, `    token_estimate: ${memory.tokenEstimate}`);
  }
...
describe("runListCommand", () => {
  ...
  test("lists memories from a category", async () => {
    await createMemoryFile("project/test-memory");
    await createIndex("project", [{ path: "project/test-memory", tokenEstimate: 10 }]);
  ...
  test("includes expired memories with --include-expired flag", async () => {
    const pastDate = new Date(Date.now() - 86400000);
  ...
  test("outputs JSON format with --format json", async () => {
    await createMemoryFile("project/test-memory");
    await createIndex("project", [{ path: "project/test-memory", tokenEstimate: 10 }]);
