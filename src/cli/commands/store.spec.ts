...
describe(
    'store CLI commands', () => {
        ...
        const buildOptions = (args: string[]) => ({
            ...
            cwd: tempDir,
            registryPath: join(
                tempDir, 'store-registry.yaml',
            ),
        });

        beforeEach(async () => {
            tempDir = await mkdtemp(join(
                tmpdir(), 'cortex-store-cli-',
            )); 
        });
        ...
        it(
            'should list an empty registry when missing', async () => {
        ...
        it(
            'should list registry entries sorted by name', async () => {
        ...
        it(
            'should add a store and persist the registry', async () => {
                ...
                const registryContents = await readFile(
                    buildOptions([]).registryPath, 'utf8',
                );
        ...
        it(
            'should reject duplicate store registration', async () => {
                const saved = await saveStoreRegistry(
                    buildOptions([]).registryPath, {
                        primary: { path: 'C:/Stores/Primary' },
                    },
                );
        ...
        it(
            'should remove a store and keep remaining entries', async () => {
                const saved = await saveStoreRegistry(
                    buildOptions([]).registryPath, {
                        primary: { path: 'C:/Stores/Primary' },
        ...
        it(
            'should delete the registry when removing the last store', async () => {
                const saved = await saveStoreRegistry(
                    buildOptions([]).registryPath, {
                        primary: { path: 'C:/Stores/Primary' },
                    },
        ...
        it(
            'should initialize a store in the default path', async () => {
        ...
        it(
            'should reject missing commands and arguments', async () => {
        ...
        it(
            'should reject invalid store inputs', async () => {
                const invalidName = await runStoreCommand(buildOptions([
                    'add',
                    'Bad Name',
                    'C:/Stores/Primary',
                ]));
