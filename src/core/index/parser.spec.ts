...
describe(
    'category index parsing and serialization', () => {
        it(
            'should round-trip serialized indexes', () => {
                const index = {
                    memories: [{
        ...
        it(
            'should reject invalid list formatting', () => {
                const raw = [
                    'memories:',
                    '  path: working/preferences',
                    '  token_estimate: 3',
        ...
        it(
            'should reject unknown index entry keys', () => {
                const raw = [
                    'memories:',
                    '  -',
                    '    path: working/preferences',
...
describe(
    'index maintenance', () => {
        ...
        it(
            'should update category indexes on memory writes', async () => {
                ...
                if (priorityParentParsed.ok) {
                    expect(priorityParentParsed.value.subcategories).toEqual([{ path: 'semantic/priorities/urgent', memoryCount: 1 }]); 
                }
        ...
        it(
            'should rebuild indexes during manual reindex', async () => {
                ...
                await mkdir(
                    join(
                        memoryRoot, 'semantic', 'concepts',
                    ), { recursive: true },
                );
                await mkdir(
                    join(
                        memoryRoot, 'semantic', 'priorities',
                    ), { recursive: true },
                );
                await mkdir(
                    join(
                        memoryRoot, 'semantic', 'priorities', 'urgent',
                    ), { recursive: true },
                );
                await writeFile(
                    join(
                        memoryRoot, 'working', 'notes.md',
                    ), 'Working notes',
                );
                await writeFile(
                    join(
                        memoryRoot, 'semantic', 'concepts', 'priority.md',
                    ),
                    'Priority content',
                );
                await writeFile(
                    join(
                        memoryRoot, 'semantic', 'priorities', 'urgent.md',
                    ), 'Urgent priority',
                );
                await writeFile(
                    join(
                        memoryRoot, 'semantic', 'priorities', 'backlog.md',
                    ),
                    'Backlog priority',
                );
                await writeFile(
                    join(
                        memoryRoot, 'semantic', 'priorities', 'urgent', 'deep.md',
                    ),
                    'Deep priority',
                );
                ...
                if (workingIndexContents.ok && workingIndexContents.value) {
                    ...
                    if (workingParsed.ok) {
                        ...
                        expect(estimate.ok).toBe(true);
                        if (estimate.ok) {
                            expect(workingParsed.value.memories).toEqual([{ path: 'working/notes', tokenEstimate: estimate.value }]); 
                        }
                    }
                }
                ...
                if (semanticIndexContents.ok && semanticIndexContents.value) {
                    ...
                    if (semanticParsed.ok) {
                        ...
                        expect(estimate.ok).toBe(true);
                        if (estimate.ok) {
                            expect(semanticParsed.value.memories).toEqual([{ path: 'semantic/concepts/priority', tokenEstimate: estimate.value }]); 
                        }
                    }
                }
        ...
        it(
            'should reject unexpected reindex args', async () => {
