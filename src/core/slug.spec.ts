...
describe('slug utilities', () => {
    describe('isValidMemorySlug', () => {
        ...
        it('should accept slugs with hyphens between segments', () => {
    ...
    describe('normalizeSlugSegments', () => {
        it('should trim whitespace from segments', () => {
            const result = normalizeSlugSegments(['  working ', ' memory  ']);
            expect(result).toEqual(['working', 'memory']);
        });
        ...
        it('should preserve valid segments unchanged', () => {
            const result = normalizeSlugSegments(['alpha', 'beta', 'gamma']);
            expect(result).toEqual(['alpha', 'beta', 'gamma']);
        });
    });

    describe('buildMemorySlugPath', () => {
        it('should join single category with slug', () => {
            const categories: MemoryCategoryPath = ['project'];
            const slug: MemorySlug = 'memory';

            const result = buildMemorySlugPath(categories, slug);

            expect(result).toBe('project/memory');
        });

        it('should join multiple categories with slug', () => {
            const categories: MemoryCategoryPath = ['semantic', 'concepts'];
        ...
        it('should handle deeply nested categories', () => {
            const categories: MemoryCategoryPath = ['a', 'b', 'c', 'd'];
    ...
    describe('buildMemoryIdentity', () => {
        it('should build identity with single category', () => {
            const categories: MemoryCategoryPath = ['project'];
            const slug: MemorySlug = 'memory';

            const result = buildMemoryIdentity(categories, slug);
        ...
        it('should build identity with multiple categories', () => {
            const categories: MemoryCategoryPath = ['semantic', 'concepts'];
            const slug: MemorySlug = 'priority-cues';
            ...
            expect(result.categories).toEqual(['semantic', 'concepts']);
            expect(result.slug).toBe('priority-cues');
        });

        it('should build identity with deeply nested categories', () => {
            const categories: MemoryCategoryPath = ['a', 'b', 'c'];
        ...
        it('should preserve category array reference independence', () => {
            const categories: MemoryCategoryPath = ['project'];
            const slug: MemorySlug = 'memory';
