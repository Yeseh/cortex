...
const parseEntryLineValue = (
  ...
  parseEntryLine(rawLine, lineNumber);

const parseTokenEstimate = (
  value: string,
  line: number
): Result<number, IndexParseError> => parseNumber(value, "token_estimate", line);

const parseMemoryCount = (
  value: string,
  line: number
): Result<number, IndexParseError> => parseNumber(value, "memory_count", line);
...
const parseEntryKeyValue = (
  ...
  parseEntryLineValue(line, lineNumber);
...
const parseIndexEntry = (
  ...
  index: number,
  section: Section
): Result<{ entry: IndexMemoryEntry | IndexSubcategoryEntry; nextIndex: number }, IndexParseError> =>
  parseEntry(lines, index + 1, 1, section);

const serializeMemoryEntry = (
  entry: IndexMemoryEntry
): Result<string[], IndexSerializeError> => {
  if (!entry.path?.trim()) {
    return err({
      code: "INVALID_ENTRY",
  ...
  const lines = ["  -", `    path: ${entry.path.trim()}`, `    token_estimate: ${parsedToken.value}`];
...
const serializeSubcategoryEntry = (
  entry: IndexSubcategoryEntry
): Result<string[], IndexSerializeError> => {
  if (!entry.path?.trim()) {
    return err({
      code: "INVALID_ENTRY",
...
const resolveSection = (
  line: string
): Result<Section | null, IndexParseError> => {
...
export const parseCategoryIndex = (
  raw: string
): Result<CategoryIndex, IndexParseError> => {
  ...
  while (index < lines.length) {
    ...
    const parsedEntry = parseIndexEntry(lines, index, section);
    if (!parsedEntry.ok) {
      return parsedEntry;
    }
...
export const serializeCategoryIndex = (
  index: CategoryIndex
): Result<string, IndexSerializeError> => {
