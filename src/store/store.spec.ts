...
describe('store resolution', () => {
    ...
    it('should prefer a local store when available', async () => {
        ...
        if (result.ok) {
            expect(result.value.root.replace(/\\/g, '/')).toBe('C:/work/project/.cortex');
    ...
    it('should fall back to the global store when local is missing', async () => {
        ...
        expect(result.ok).toBe(true);
        if (result.ok) {
            expect(result.value.root.replace(/\\/g, '/')).toBe('C:/global/.cortex');
    ...
    it('should resolve relative global paths against cwd', async () => {
        ...
        expect(result.ok).toBe(true);
        if (result.ok) {
            expect(result.value.root.replace(/\\/g, '/')).toBe('C:/work/missing/stores/.cortex');
            expect(result.value.scope).toBe('global');
        }
    });
    ...
    it('should reject missing local store when strict_local is enabled', async () => {
        ...
        if (!result.ok) {
            ...
            expect(result.error.path?.replace(/\\/g, '/')).toBe('C:/work/missing/.cortex');
        }
    });
    ...
    it('should reject missing global store when local is absent', async () => {
        ...
        if (!result.ok) {
            ...
            expect(result.error.path?.replace(/\\/g, '/')).toBe('C:/global/missing');
        }
    });
    ...
    it('should surface access errors for stores', async () => {
        ...
        if (!result.ok) {
            ...
            expect(result.error.path?.replace(/\\/g, '/')).toBe('C:/work/error/.cortex');
        }
    });
});
