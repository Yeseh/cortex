...
describe('Cortex CLI Integration Tests', () => {
  ...
  describe('add command', () => {
    ...
    it('should fail for unknown flags', async () => {
      const result = await runCortexCli(['add', 'project/memory', '--unknown-flag', 'value'], {
        globalStorePath: testStore,
      });
    ...
    it('should add memory from file', async () => {
      ...
      const result = await runCortexCli(['add', 'project/file-memory', '--file', contentFile], {
        globalStorePath: testStore,
      });
    ...
    it('should add memory in deeply nested category', async () => {
      const result = await runCortexCli(
        ['add', 'domain/subdomain/feature/deep-memory', '--content', 'Deeply nested content.'],
        { globalStorePath: testStore }
      );
  ...
  describe('update command', () => {
    ...
    it('should update content from file', async () => {
      ...
      const result = await runCortexCli(['update', 'project/updatable', '--file', contentFile], {
        globalStorePath: testStore,
      });
    ...
    it('should preserve original content when only updating tags', async () => {
      const result = await runCortexCli(['update', 'project/updatable', '--tags', 'new-tag'], {
        globalStorePath: testStore,
      });
