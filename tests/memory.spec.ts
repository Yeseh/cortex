...
import { normalizeSlugSegments } from "../src/core/slug.ts";
import { parseMemoryFile, serializeMemoryFile } from "../src/memory/file.ts";
import {
  validateCategoryPath,
  validateMemorySlugPath,
} from "../src/memory/validation.ts";
...
describe("memory file parsing", () => {
  it("should parse frontmatter with required fields and optional expiry", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.frontmatter.createdAt.toISOString()).toBe(
        "2024-01-01T00:00:00.000Z"
      );
      expect(result.value.frontmatter.updatedAt.toISOString()).toBe(
        "2024-01-02T00:00:00.000Z"
      );
      expect(result.value.frontmatter.expiresAt?.toISOString()).toBe(
        "2024-02-01T00:00:00.000Z"
      );
      expect(result.value.frontmatter.tags).toEqual(["personal", "onboarding"]);
  ...
  it("should parse tags in list style", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.frontmatter.tags).toEqual(["product", "research"]);
  ...
  it("should reject empty tags in inline lists", () => {
    const raw = [
      "---",
      "created_at: 2024-01-01T00:00:00.000Z",
      "updated_at: 2024-01-02T00:00:00.000Z",
...
describe("memory file serialization", () => {
  it("should serialize and re-parse consistently", () => {
    ...
    if (reparsed.ok) {
      expect(reparsed.value.frontmatter.createdAt.toISOString()).toBe(
        "2024-03-01T08:30:00.000Z"
      );
      expect(reparsed.value.frontmatter.updatedAt.toISOString()).toBe(
        "2024-03-02T10:15:00.000Z"
      );
      expect(reparsed.value.frontmatter.expiresAt?.toISOString()).toBe(
        "2024-04-01T00:00:00.000Z"
      );
      expect(reparsed.value.frontmatter.tags).toEqual(["alpha", "beta"]);
  ...
  it("should reject non-array tags during serialization", () => {
    ...
    const result = serializeMemoryFile(
      memory as unknown as Parameters<typeof serializeMemoryFile>[0]
    );
