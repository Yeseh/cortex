...
import { defaultTokenizer } from "../src/core/tokens.ts";
import { parseCategoryIndex, serializeCategoryIndex } from "../src/index/parser.ts";
import { FilesystemStorageAdapter } from "../src/storage/filesystem.ts";
import { runReindexCommand } from "../src/cli/commands/reindex.ts";
...
describe("index maintenance", () => {
  ...
  it("should update category indexes on memory writes", async () => {
    ...
    const priorityIndexContents = await adapter.readIndexFile(
      "semantic/priorities"
    );
    ...
    if (priorityParsed.ok) {
      ...
      const backlogEstimate = defaultTokenizer.estimateTokens(
        semanticPriorityBacklogContent
      );
      expect(estimate.ok).toBe(true);
    ...
    const updatedParentParsed = parseCategoryIndex(
      updatedParentIndexContents.value
    );
    ...
    const updatedWorkingParsed = parseCategoryIndex(
      updatedWorkingIndexContents.value
    );
  ...
  it("should rebuild indexes during manual reindex", async () => {
    ...
    await writeFile(
      join(memoryRoot, "semantic", "priorities", "urgent.md"),
      "Urgent priority"
    );
    await writeFile(
