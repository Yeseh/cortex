...
// ============================================================================
// Mock Memory Serializer
// ============================================================================

/**
 * Creates a mock serializer that parses/serializes YAML frontmatter format.
 * This is a simplified implementation for testing purposes.
 */
const createMockSerializer = (): MemorySerializer => ({
    ...
    serialize: (memory: Memory): Result<string, MemoryError> => {
        const lines = ['---'];
        lines.push(`created_at: ${memory.metadata.createdAt.toISOString()}`);
        lines.push(`updated_at: ${memory.metadata.updatedAt.toISOString()}`);
        lines.push(`tags: [${memory.metadata.tags.join(', ')}]`);
        lines.push(`source: ${memory.metadata.source}`);
        if (memory.metadata.expiresAt) {
            lines.push(`expires_at: ${memory.metadata.expiresAt.toISOString()}`);
        }
        lines.push('---');
...
describe('createMemory', () => {
    ...
    it('should return INVALID_PATH for malformed path', async () => {
        const storage = createMockStorage();
        const result = await createMemory(storage, mockSerializer, 'invalid', {
            content: 'Test content',
            source: 'test',
        });
    ...
    it('should return INVALID_PATH for empty path', async () => {
        const storage = createMockStorage();
        const result = await createMemory(storage, mockSerializer, '', {
            content: 'Test content',
            source: 'test',
        });
        expect(result.ok).toBe(false);
...
describe('getMemory', () => {
    ...
    it('should return MEMORY_EXPIRED when expired and includeExpired=false', async () => {
        ...
        const result = await getMemory(storage, mockSerializer, 'project/test/memory', { now });
        expect(result.ok).toBe(false);
        if (!result.ok) {
    ...
    it('should return INVALID_PATH for malformed path', async () => {
        const storage = createMockStorage();
        const result = await getMemory(storage, mockSerializer, 'invalid');
        expect(result.ok).toBe(false);
        if (!result.ok) {
            expect(result.error.code).toBe('INVALID_PATH');
        }
...
subcategories: []`;
        ...
        const result = await listMemories(storage, mockSerializer, { category: 'project/test' });
        expect(result.ok).toBe(true);
        if (result.ok) {
    ...
    it('should return empty results for empty category', async () => {
        const storage = createMockStorage();
        const result = await listMemories(storage, mockSerializer, { category: 'project/empty' });
        expect(result.ok).toBe(true);
        if (result.ok) {
...
subcategories: []`;
        ...
        const now = new Date('2025-06-15T12:00:00Z');
        const result = await pruneExpiredMemories(storage, mockSerializer, { dryRun: true, now });
        expect(result.ok).toBe(true);
