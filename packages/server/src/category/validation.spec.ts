...
describe('Category Descriptions Validation', () => {
    describe('Root category protection', () => {
        ...
        it('setDescription should allow root category "project"', async () => {
            // First create the root category
            await createCategoryHandler({ config }, { store: STORE, path: 'project/temp' });

            const result = await setCategoryDescriptionHandler(
                { config },
                { store: STORE, path: 'project', description: 'Project category description' },
            );
            const output = JSON.parse(result.content[0]!.text);

            expect(output.description).toBe('Project category description');
        });

        it('setDescription should allow root category "human"', async () => {
            // First create the root category
            await createCategoryHandler({ config }, { store: STORE, path: 'human/temp' });

            const result = await setCategoryDescriptionHandler(
                { config },
                { store: STORE, path: 'human', description: 'Human category description' },
            );
            const output = JSON.parse(result.content[0]!.text);

            expect(output.description).toBe('Human category description');
        });

        it('setDescription on root should write to store root index', async () => {
            // Create a root category by creating a subcategory first
            await createCategoryHandler({ config }, { store: STORE, path: 'domain/test' });

            // Set description on root category
            await setCategoryDescriptionHandler(
                { config },
                { store: STORE, path: 'domain', description: 'Domain knowledge' },
            );

            // Verify by reading the root index file directly
